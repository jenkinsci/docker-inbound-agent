            ...
            parallel {
                stage('Windows') {
                    matrix {
                        ...
                        stages {
                            stage('Windows Build, Test and Publish') {
                                steps {
                                    powershell "& ./make.ps1 -Build ${CONTAINER_NAME} test"
                                    script {
                                        ...
                                        if(tagName =~ /\d(\.\d)+(-\d+)?/) {
                                            ...
                                            infra.withDockerCredentials {
                                                powershell "& ./make.ps1 -Build ${CONTAINER_NAME} -PushVersions -VersionTag $tagName publish"
                                            }
                                        }
                                    }
                                }
